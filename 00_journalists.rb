...
shortest_twitter = liste.min_by { |word| word.length }
puts shortest_twitter


five_liste = liste.select { |e| e.match(/\b\w{5}\b/) }

puts five_liste

liste.map! { |e| e.delete_prefix("@") }

cb_majuscule = liste.count { |e| e.match(/^[A-Z]/) }

puts cb_majuscule

liste.map! { |e| "@"+e }

sorted_list = liste.sort
 puts sorted_list

length_list = liste.sort_by { |e| e.length }

puts length_list

place_epenser = liste.index("@epenser")

puts place_epenser

repart_length = liste.inject(Hash.new(0)){|h, x| h[x.length] +=1; h}
